...
} from '\var\task\free-website-design-today/.netlify/dist/run/handlers/request-context.cjs'
import { getTracer, withActiveSpan } from '\var\task\free-website-design-today/.netlify/dist/run/handlers/tracer.cjs'

process.chdir('\var\task\free-website-design-today')
  ...
  const handlerResponse = await runWithRequestContext(requestContext, () => {
    return withActiveSpan(tracer, 'Next.js Server Handler', async (span) => {
      span?.setAttributes({
        ...
        monorepo: true,
        cwd: '\var\task\free-website-design-today',
      })
      if (!cachedHandler) {
        const { default: handler } = await import('\var\task\free-website-design-today\.netlify\dist\run\handlers\server.js')
